...
getsigresult <- function(signature_score, signature_pval){
  p <- rownames(sig_score)
  output <- data.frame(rep(p, ncol(sig_score)))
  sig_name <- NULL
  for(i in colnames(sig_score)){
    sig_name <- rbind(sig_name, data.frame(rep(i, nrow(sig_score))))
  }
  sig_sc <- NULL
  for (n in 1:ncol(sig_score)) {
    sig_sc <- rbind(sig_sc, data.frame(sig_score[,n]))
  }
  sig_p <- NULL
  for (n in 1:ncol(sig_pval)) {
    sig_p <- rbind(sig_p, data.frame(sig_pval[,n]))
  }
